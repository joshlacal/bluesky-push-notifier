...
const PROD_ROOT_CERT_URLS: &[&str] = &[
    "https://www.apple.com/certificateauthority/Apple_App_Attestation_Root_CA.pem",
];
const DEV_ROOT_CERT_URLS: &[&str] = &[
    "https://www.apple.com/certificateauthority/Apple_App_Attestation_Root_CA.pem",
];

/// Identifies which Apple App Attest environment issued the attestation materials.
...
impl AttestationEnvironment {
    fn root_cert_urls(self) -> &'static [&'static str] {
        match self {
            Self::Production => PROD_ROOT_CERT_URLS,
            Self::Development => DEV_ROOT_CERT_URLS,
        }
    }
}
...
impl Attestation {
   ...
   /// Fetches the Apple root certificate from the specified URL.
    fn fetch_apple_root_cert(urls: &[&str]) -> Result<X509, AppAttestError> {
        let client = Client::builder()
            .timeout(Duration::from_secs(30))
            .build()
            .map_err(|e| AppAttestError::Message(format!("Failed to build HTTP client: {}", e)))?;

        let mut last_error: Option<AppAttestError> = None;

        for url in urls {
            tracing::debug!(url = *url, "Attempting to download App Attest root certificate");

            match client.get(*url).send() {
                Ok(response) => {
                    if !response.status().is_success() {
                        tracing::warn!(status = ?response.status(), "Root certificate download failed");
                        last_error = Some(AppAttestError::Message(format!(
                    ...
                    match response.bytes() {
                        Ok(bytes) => {
                            if let Ok(cert) = X509::from_pem(&bytes) {
                                tracing::debug!(url = *url, "Parsed App Attest root certificate as PEM");
                                return Ok(cert);
                            }

                            if let Ok(cert) = X509::from_der(&bytes) {
                                tracing::debug!(url = *url, "Parsed App Attest root certificate as DER");
                                return Ok(cert);
                            }

                            tracing::warn!(url = *url, "Unable to parse App Attest root certificate");
                            last_error = Some(AppAttestError::Message(
                                "Failed to parse certificate".to_string(),
                            ));
                        }
                        Err(e) => {
                            tracing::warn!(url = *url, error = %e, "Failed reading certificate bytes");
                            last_error = Some(AppAttestError::Message(format!(
                ...
                Err(e) => {
                    tracing::warn!(url = *url, error = %e, "Root certificate request failed");
                    last_error = Some(AppAttestError::Message(format!(
        ...
        Err(last_error.unwrap_or_else(|| {
            AppAttestError::Message("Unable to download App Attest root certificate".to_string())
        }))
    }

    /// verifyCertificates verifies the certificate chain in the attestation statement
    fn log_certificate_debug(cert: &X509, label: &str) {
        if let Ok(subject) = cert
            .subject_name()
            .entries()
            .next()
            .map(|e| e.data().as_utf8())
            .transpose()
        {
            tracing::debug!("{label} subject={:?}", subject);
        }

        if let Ok(fp) = cert.digest(MessageDigest::sha256()) {
            let hex = hex::encode_upper(fp);
            tracing::debug!("{label} sha256={hex}");
        }
    }
    ...
    fn verify_certificates(certificates: Vec<Vec<u8>>, apple_root_cert: &X509) -> Result<(), Box<dyn Error>> {
        ...
        for cert_der in certificates {
            ...
            Self::log_certificate_debug(&cert, "attestation-chain");
            certs.push(cert);
        }

        let mut store_builder = X509StoreBuilder::new()?;

        Self::log_certificate_debug(apple_root_cert, "trusted-root");
    ...
    fn client_data_hash(challenge: &str) -> Result<Vec<u8>, AppAttestError> {
        // CRITICAL FIX: Handle both raw challenge and JSON client data formats
        // The client now sends JSON client data, so we need to handle that case

        // First, try to parse as JSON client data (new format)
        if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(challenge) {
            if let Some(json_challenge) = json_value.get("challenge").and_then(|c| c.as_str()) {
                println!("🔍 ATTESTATION: Found JSON client data with challenge: {}", json_challenge);
                // Hash the entire JSON string (what the client actually hashed)
                let mut hasher = Sha256::new();
                hasher.update(challenge.as_bytes());
                let hash = hasher.finish().as_slice().to_vec();
                println!("🔍 ATTESTATION: JSON client data hash: {:?}", hash);
                return Ok(hash);
            }
        }

        // Fallback to original logic for raw challenge format
        println!("🔍 ATTESTATION: Using raw challenge format");

        // Attempt to decode the challenge using the variants App Attest clients commonly emit.
        // Apple provides a URL-safe base64 string, though some SDKs still send standard base64.
        let decoded = general_purpose::STANDARD
            .decode(challenge)
            .or_else(|_| general_purpose::URL_SAFE.decode(challenge))
            .or_else(|_| general_purpose::URL_SAFE_NO_PAD.decode(challenge))
            .or_else(|_| general_purpose::STANDARD_NO_PAD.decode(challenge))
            .map_err(|e| AppAttestError::Message(format!("Failed to decode App Attest challenge: {}", e)))?;

        if decoded.len() != 32 {
            return Err(AppAttestError::Message(format!(
                "App Attest challenge must decode to 32 bytes, got {}",
                decoded.len()
            )));
        }
        ...
        let hash = hasher.finish().as_slice().to_vec();
        println!("🔍 ATTESTATION: Raw challenge hash: {:?}", hash);
        Ok(hash)
    }
    ...
    pub fn verify(self, challenge: &str, app_id: &str, key_id: &str) -> Result<(Vec<u8>, Vec<u8>),  Box<dyn Error>> {
        self.verify_with_environment(challenge, app_id, key_id, AttestationEnvironment::Production)
    }

    pub fn verify_with_environment(
        self,
        challenge: &str,
        app_id: &str,
        key_id: &str,
        ...
        self.verify_with_root(challenge, app_id, key_id, environment.root_cert_urls())
    }
    ...
    pub fn verify_with_root(
        ...
        let apple_root_cert = Attestation::fetch_apple_root_cert(root_cert_urls)?;
        self.verify_with_root_cert(challenge, app_id, key_id, &apple_root_cert)
    }

    fn verify_with_root_cert(
        self,
        challenge: &str,
        app_id: &str,
        key_id: &str,
