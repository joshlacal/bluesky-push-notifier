...
impl AuthenticatorData {
    ...
    pub(crate) fn is_valid_aaguid(&self) -> bool {
        if let Some(aaguid) = &self.aaguid {
            tracing::debug!("🔍 AAGUID validation - parsed AAGUID: {:?}", aaguid.0);
            tracing::debug!("🔍 AAGUID validation - parsed AAGUID bytes: {:?}", aaguid.bytes());

            let aaguid_bytes = aaguid.bytes();

            // Check against all known Apple App Attest AAGUIDs
            for &expected_id in APPLE_APP_ATTEST_AAGUIDS.iter() {
                let expected_bytes = expected_id.as_bytes().to_vec();
                tracing::debug!("🔍 AAGUID validation - checking against: {} (bytes: {:?})", expected_id, expected_bytes);

                if aaguid_bytes == expected_bytes {
                    tracing::debug!("🔍 AAGUID validation - MATCH found for: {}", expected_id);
                    return true;
                }
            }

            tracing::debug!("🔍 AAGUID validation - NO MATCH found for any known AAGUID");
            return false;
        }

        tracing::debug!("🔍 AAGUID validation - no AAGUID found");
        false
    }
...
// Additional known Apple App Attest AAGUIDs based on research
const APPLE_APP_ATTEST_AAGUIDS: &[&str] = &[
    "appattest",         // Original production
    "appattestdevelop",  // Original development
    "appattestdev",      // Shorter development variant
    "apple-appattest",   // With prefix
    "apple-appattestdev", // With prefix development
];

impl AAGUID {
    fn new(b: Vec<u8>) -> Result<Self, AppAttestError> {
        tracing::debug!("🔍 AAGUID::new - raw bytes ({} bytes): {:?}", b.len(), b);
        tracing::debug!("🔍 AAGUID::new - raw bytes as string: {:?}", String::from_utf8_lossy(&b));

        let trimmed = AAGUID::trim_trailing_zeros(b.as_slice());
        tracing::debug!("🔍 AAGUID::new - trimmed bytes ({} bytes): {:?}", trimmed.len(), trimmed);
        tracing::debug!("🔍 AAGUID::new - trimmed as string: {:?}", String::from_utf8_lossy(trimmed));

        for &id in APPLE_APP_ATTEST_AAGUIDS.iter() {
            tracing::debug!("🔍 AAGUID::new - checking against id: {} (bytes: {:?})", id, id.as_bytes());
            if id.as_bytes() == trimmed {
                tracing::debug!("🔍 AAGUID::new - MATCH found for: {}", id);
                return Ok(AAGUID(id.to_string()));
            }
        }

        tracing::debug!("🔍 AAGUID::new - NO MATCH found, returning InvalidAAGUID");
